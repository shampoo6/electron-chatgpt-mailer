...
export async function makeZipFile(templateParams: { [key: string]: any }): Promise<void> {
    // 判断是否有关联参数
    if ('relative' in templateParams) {
        requires.push(`笔记的内容和 ${templateParams.relative} 有关`)
    } else {
        requires.push('笔记的内容和这些有关: web技术，jquery，canvas，微信小程序，uni-app，node.js，mongodb，mysql，express，animate.css，react，bootstrap，vue')
        requires.push('请随机选择上述技术中的一种进行书写')
    }

    // 构造prompts
    let prompts = [...chatConfig.requires, ...requires].map((require, i) => `${i + 1}. ${require};`).join('\n')
    prompts = `请帮我编辑一段HTML代码，要求如下:
${prompts}
模板如下:
<!DOCTYPE html>
<html>
